...
V 0.5 Auto tagging for new posts, small modifications here and there..<br />V 0.6 Now you are able to match whole words only!! , Fixed a bug where the first word in a post is not detected.<br /> V 0.7 TagGator now searchs for tags in the titles too, you are now able to see the number of tags applied when manually tagging <br /> Added a nice Readme.txt and releasing version 1.0!!< br /> V 1.1 Wordpress now updates the tags count automatically, fixed a bug when case sensetivity is enabled.
Author: PsMan
Version: 1.1
...
if ($mhw && ((!$cst && (preg_match("/\b$atag\b/iu", strtolower($aa)))) || ($cst && (preg_match("/\b$atag\b/u", ($aa)))))|| !$mhw && (!$cst && (strpos(strtolower($aa), strtolower($atag))) || ($cst && (strpos
                    (($aa), ($atag)))))) {
...
";
    ...
    echo <<< EOST
    <div id='message' class='error'><p>If you find my work useful, <a href="http://angrybyte.com/donate" style="color:red;">please consider making a donation</a>, any amount is welcome and highly appreciated.<br /></h2></p></div>
<div id="icon-edit" class="icon32"><br /></div><h2>TagGator</h2>
TagGator will search for the keywords you write below through your posts and add these keywords as tags to the returned posts. For more info <a href="http://angrybyte.com/wordpress-plugins/taggator">visit TagGator's homepage</a> <br /> 
...
<p style="color:red;">$txtmat</p> <br><br>
...
EOST;
    ...
    echo "<Form method ='post' action='$serv'><textarea name='xx' cols='50' rows='10'>$oldtags</textarea><input type='submit' value='submit'> <br /> <input type='checkbox' name='cst' $chkd />Case Sensetive? <br /> <input type='checkbox' name='mhw' $chkd2 />Match whole words only? </form><br /><br /> If you don't like the result, Manually remove the added tags from the tags page under 'Posts'<br><br>";
